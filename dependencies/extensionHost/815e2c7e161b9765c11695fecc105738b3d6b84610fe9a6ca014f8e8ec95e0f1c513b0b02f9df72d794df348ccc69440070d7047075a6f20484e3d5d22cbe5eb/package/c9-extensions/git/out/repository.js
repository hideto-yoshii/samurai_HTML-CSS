...
class FileEventLogger {
    constructor(onWorkspaceWorkingTreeFileChange, onDotGitFileChange, outputChannel) {
        ...
        this.logLevelDisposable = vscode_1.env.onDidChangeLogLevel(this.onDidChangeLogLevel, this);
        this.onDidChangeLogLevel(vscode_1.env.logLevel);
    }
    onDidChangeLogLevel(level) {
        this.eventDisposable.dispose();
        if (level > vscode_1.LogLevel.Debug) {
            return;
        }
...
class DotGitWatcher {
    ...
    updateTransientWatchers() {
        ...
        try {
            ...
            if (vscode_1.env.logLevel <= vscode_1.LogLevel.Error) {
...
class Repository {
    ...
    async cleanUpCommitEditMessage(editMessage) {
