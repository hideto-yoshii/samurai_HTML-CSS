...
define(["require", "exports", "assert", "os", "vs/base/common/path", "fs", "stream", "vs/base/common/uuid", "vs/base/node/pfs", "vs/base/common/async", "vs/base/common/amd", "vs/base/common/cancellation", "vs/base/common/platform", "vs/base/common/normalization", "vs/base/common/buffer", "path"], function (require, exports, assert, os, path, fs, stream_1, uuid, pfs, async_1, amd_1, cancellation_1, platform_1, normalization_1, buffer_1, path_1) {
    ...
    suite('PFS', () => {
        ...
        test('writeFile - parallel write on different files works', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
        ...
        test('writeFile - parallel write on same files works and is sequentalized', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            const testFile = path.join(newDir, 'writefile.txt');
        ...
        test('rimraf - simple - unlink', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('rimraf - simple - move', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('rimraf - recursive folder structure - unlink', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('rimraf - recursive folder structure - move', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('rimraf - simple ends with dot - move', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = `${uuid.generateUuid()}.`;
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('rimraf - simple ends with dot slash/backslash - move', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = `${uuid.generateUuid()}.`;
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('rimrafSync - simple', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('rimrafSync - recursive folder structure', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('moveIgnoreError', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('copy, move and delete', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
        ...
        test('mkdirp', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('mkdirp cancellation', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
        ...
        test('readDirsInDir', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            yield pfs.mkdirp(newDir, 493);
        ...
        test('stat link', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        test('readdir', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        test('readdirWithFileTypes', () => __awaiter(void 0, void 0, void 0, function* () {
            if (normalization_1.canNormalize && typeof process.versions['electron'] !== 'undefined' /* needs electron */) {
                const id = uuid.generateUuid();
                const parentDir = path.join(os.tmpdir(), 'vsctests', id);
                ...
                yield pfs.mkdirp(newDir, 493);
                yield pfs.writeFile(path_1.join(testDir, 'somefile.txt'), 'contents');
                assert.ok(fs.existsSync(newDir));
                const children = yield pfs.readdirWithFileTypes(testDir);
        ...
        test('writeFile (string)', () => __awaiter(void 0, void 0, void 0, function* () {
            const smallData = 'Hello World';
            const bigData = (new Array(100 * 1024)).join('Large String\n');
            return testWriteFileAndFlush(smallData, smallData, bigData, bigData);
        }));
        test('writeFile (Buffer)', () => __awaiter(void 0, void 0, void 0, function* () {
            const smallData = 'Hello World';
            const bigData = (new Array(100 * 1024)).join('Large String\n');
            return testWriteFileAndFlush(Buffer.from(smallData), smallData, Buffer.from(bigData), bigData);
        }));
        test('writeFile (UInt8Array)', () => __awaiter(void 0, void 0, void 0, function* () {
            const smallData = 'Hello World';
            const bigData = (new Array(100 * 1024)).join('Large String\n');
        ...
        test('writeFile (stream)', () => __awaiter(void 0, void 0, void 0, function* () {
            const smallData = 'Hello World';
            const bigData = (new Array(100 * 1024)).join('Large String\n');
        ...
        test('writeFile (file stream)', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
        ...
        test('writeFile (string, error handling)', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            const testFile = path.join(newDir, 'flushed.txt');
        ...
        test('writeFile (stream, error handling EISDIR)', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            const testFile = path.join(newDir, 'flushed.txt');
        ...
        test('writeFile (stream, error handling READERROR)', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            const testFile = path.join(newDir, 'flushed.txt');
        ...
        test('writeFile (stream, error handling EACCES)', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        test('writeFile (file stream, error handling)', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const sourceFile = amd_1.getPathFromAmdModule(require, './fixtures/index.html');
            const newDir = path.join(parentDir, 'pfs', id);
        ...
        test('writeFileSync', () => __awaiter(void 0, void 0, void 0, function* () {
            const id = uuid.generateUuid();
            const parentDir = path.join(os.tmpdir(), 'vsctests', id);
            const newDir = path.join(parentDir, 'pfs', id);
            const testFile = path.join(newDir, 'flushed.txt');
