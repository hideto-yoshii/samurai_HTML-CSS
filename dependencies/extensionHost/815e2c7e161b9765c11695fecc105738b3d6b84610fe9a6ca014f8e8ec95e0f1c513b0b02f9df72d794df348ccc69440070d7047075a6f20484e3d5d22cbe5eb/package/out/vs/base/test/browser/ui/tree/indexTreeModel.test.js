...
define(["require", "exports", "assert", "vs/base/common/iterator", "vs/base/browser/ui/tree/indexTreeModel"], function (require, exports, assert, iterator_1, indexTreeModel_1) {
    ...
    suite('IndexTreeModel', function () {
        test('ctor', () => {
            const list = [];
            const model = new indexTreeModel_1.IndexTreeModel('test', toSpliceable(list), -1);
            assert(model);
        ...
        test('simple filter', function () {
            ...
            const model = new indexTreeModel_1.IndexTreeModel('test', toSpliceable(list), -1, { filter });
            model.splice([0], 0, iterator_1.Iterator.fromArray([
                {
                    element: 0, children: [
                        { element: 1 },
        ...
        test('recursive filter', function () {
            ...
            const model = new indexTreeModel_1.IndexTreeModel('test', toSpliceable(list), 'root', { filter });
            model.splice([0], 0, iterator_1.Iterator.fromArray([
                {
        ...
        test('refilter with filtered out nodes', function () {
            ...
            model.splice([0], Number.POSITIVE_INFINITY, iterator_1.Iterator.fromArray([
                { element: 'silver' },
                { element: 'gold' },
                { element: 'platinum' }
            ]));
