...
define(["require", "exports", "assert", "vs/base/common/arrays"], function (require, exports, assert, arrays) {
    ...
    suite('Arrays', () => {
        ...
        test('topAsync', () => __awaiter(void 0, void 0, void 0, function* () {
            const cmp = (a, b) => {
                assert.strictEqual(typeof a, 'number', 'typeof a');
                assert.strictEqual(typeof b, 'number', 'typeof b');
                return a - b;
            };
            yield testTopAsync(cmp, 1);
            return testTopAsync(cmp, 2);
        }));
        function testTopAsync(cmp, m) {
            return __awaiter(this, void 0, void 0, function* () {
                {
                    const result = yield arrays.topAsync([], cmp, 1, m);
                    assert.deepEqual(result, []);
                }
                {
                    const result = yield arrays.topAsync([1], cmp, 0, m);
                    assert.deepEqual(result, []);
                }
                {
                    const result = yield arrays.topAsync([1, 2], cmp, 1, m);
                    assert.deepEqual(result, [1]);
                }
                {
                    const result = yield arrays.topAsync([2, 1], cmp, 1, m);
                    assert.deepEqual(result, [1]);
                }
                {
                    const result = yield arrays.topAsync([1, 3, 2], cmp, 2, m);
                    assert.deepEqual(result, [1, 2]);
                }
                {
                    const result = yield arrays.topAsync([3, 2, 1], cmp, 3, m);
                    assert.deepEqual(result, [1, 2, 3]);
                }
                {
                    const result = yield arrays.topAsync([4, 6, 2, 7, 8, 3, 5, 1], cmp, 3, m);
                    assert.deepEqual(result, [1, 2, 3]);
                }
            });
        }
