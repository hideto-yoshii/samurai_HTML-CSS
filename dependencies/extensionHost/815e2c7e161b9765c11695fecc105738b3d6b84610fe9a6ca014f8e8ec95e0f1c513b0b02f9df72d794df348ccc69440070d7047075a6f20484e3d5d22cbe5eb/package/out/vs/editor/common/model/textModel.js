    ...
    class TextModel extends lifecycle_1.Disposable {
        ...
        _validatePosition(_lineNumber, _column, strict) {
            ...
            if (strict) {
                const [charStartOffset,] = strings.getCharContainingOffset(this._buffer.getLineContent(lineNumber), column - 1);
                if (column !== charStartOffset + 1) {
                    return new position_1.Position(lineNumber, charStartOffset + 1);
                }
            }
            ...
            if (startColumn < startLineContent.length + 1) {
                ...
                if (startColumn !== charStartOffset + 1) {
                    if (isEmpty) {
                        ...
                        return new range_1.Range(startLineNumber, charStartOffset + 1, startLineNumber, charStartOffset + 1);
                    }
        ...
        _applyEdits(rawOperations) {
            ...
            if (contentChanges.length !== 0) {
                ...
                for (let i = 0, len = contentChanges.length; i < len; i++) {
                    ...
                    const [eolCount, firstLineLength] = tokensStore_1.countEOL(change.text);
        ...
        findEnclosingBrackets(_position) {
            const position = this.validatePosition(_position);
            const lineCount = this.getLineCount();
            let counts = [];
            const resetCounts = (modeBrackets) => {
                counts = [];
