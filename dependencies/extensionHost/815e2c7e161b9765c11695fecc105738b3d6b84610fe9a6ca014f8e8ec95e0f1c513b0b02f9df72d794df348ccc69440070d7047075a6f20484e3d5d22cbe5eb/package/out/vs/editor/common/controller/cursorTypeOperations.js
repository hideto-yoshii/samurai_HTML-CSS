    ...
    class TypeOperations {
        ...
        static _simplePaste(config, model, selections, text, pasteOnNewLine) {
            ...
            for (let i = 0, len = selections.length; i < len; i++) {
                ...
                if (pasteOnNewLine) {
                    ...
                    commands[i] = new replaceCommand_1.ReplaceCommandThatPreservesSelection(typeSelection, text, selection);
                }
                else {
        ...
        static _isAutoClosingOvertype(config, model, selections, autoClosedCharacters, ch) {
            ...
            for (let i = 0, len = selections.length; i < len; i++) {
                ...
                if (beforeCharacter === 92 /* Backslash */) {
                    return false;
                }
        ...
        static typeWithInterceptors(prevEditOperationType, config, model, selections, autoClosedCharacters, ch) {
            if (ch === '\n') {
                let commands = [];
                for (let i = 0, len = selections.length; i < len; i++) {
