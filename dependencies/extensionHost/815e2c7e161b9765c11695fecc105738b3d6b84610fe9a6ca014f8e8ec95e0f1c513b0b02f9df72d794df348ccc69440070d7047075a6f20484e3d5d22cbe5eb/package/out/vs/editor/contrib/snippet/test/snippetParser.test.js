define(["require", "exports", "assert", "vs/editor/contrib/snippet/snippetParser"], function (require, exports, assert, snippetParser_1) {
    ...
    suite('SnippetParser', () => {
        ...
        test('Snippet cannot escape closing bracket inside conditional insertion variable replacement #78883', function () {
            ...
            assert.equal(variable.transform.children[0].ifValue, 'import { hello } from world');
            assert.equal(variable.transform.children[0].elseValue, undefined);
        });
        test('Snippet escape backslashes inside conditional insertion variable replacement #80394', function () {
            ...
            assert.equal(variable.transform.children.length, 1);
            assert.ok(variable.transform.children[0] instanceof snippetParser_1.FormatString);
            assert.equal(variable.transform.children[0].ifValue, '\\');
            assert.equal(variable.transform.children[0].elseValue, undefined);
        });
    });
});
