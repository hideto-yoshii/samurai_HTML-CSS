    ...
    let MainThreadLanguageFeatures = MainThreadLanguageFeatures_1 = class MainThreadLanguageFeatures {
        ...
        static _inflateSuggestDto(defaultRange, data) {
            return {
                ...
                range: data["j" /* range */] || defaultRange,
        ...
        $registerCallHierarchyProvider(handle, selector) {
            this._registrations.set(handle, callh.CallHierarchyProviderRegistry.register(selector, {
                prepareCallHierarchy: (document, position, token) => __awaiter(this, void 0, void 0, function* () {
                    const result = yield this._proxy.$prepareCallHierarchy(handle, document.uri, position, token);
                    if (!result) {
                        return undefined;
                    }
                    return {
                        dispose: () => this._proxy.$releaseCallHierarchy(handle, result.sessionId),
                        root: MainThreadLanguageFeatures_1._reviveCallHierarchyItemDto(result.root)
                    };
                }),
                provideOutgoingCalls: (item, token) => __awaiter(this, void 0, void 0, function* () {
                    const outgoing = yield this._proxy.$provideCallHierarchyOutgoingCalls(handle, item.id, token);
                    if (!outgoing) {
                        return outgoing;
                    }
                    return outgoing.map(([item, fromRanges]) => {
                        return {
                            to: MainThreadLanguageFeatures_1._reviveCallHierarchyItemDto(item),
                            fromRanges
                        };
                    });
                }),
                provideIncomingCalls: (item, token) => __awaiter(this, void 0, void 0, function* () {
                    const incoming = yield this._proxy.$provideCallHierarchyIncomingCalls(handle, item.id, token);
                    if (!incoming) {
                        return incoming;
                    }
                    return incoming.map(([item, fromRanges]) => {
                        return {
                            from: MainThreadLanguageFeatures_1._reviveCallHierarchyItemDto(item),
                            fromRanges
                        };
                    });
                })
