    ...
    let MainThreadFileSystem = MainThreadFileSystem_1 = class MainThreadFileSystem {
        ...
        $stat(uri) {
            return this._fileService.resolve(uri_1.URI.revive(uri), { resolveMetadata: true }).then(stat => {
                return {
                    ...
                    mtime: stat.mtime,
                    size: stat.size,
                    type: MainThreadFileSystem_1._getFileType(stat)
                };
        ...
        $readdir(uri) {
            return this._fileService.resolve(uri_1.URI.revive(uri), { resolveMetadata: false }).then(stat => {
                ...
                return !stat.children ? [] : stat.children.map(child => [child.name, MainThreadFileSystem_1._getFileType(child)]);
            }).catch(MainThreadFileSystem_1._handleError);
        }
        static _getFileType(stat) {
            return (stat.isDirectory ? files_1.FileType.Directory : files_1.FileType.File) + (stat.isSymbolicLink ? files_1.FileType.SymbolicLink : 0);
        }
        $readFile(uri) {
        ...
        $writeFile(uri, content) {
            return this._fileService.writeFile(uri_1.URI.revive(uri), content).catch(MainThreadFileSystem_1._handleError);
        }
        $rename(source, target, opts) {
            return this._fileService.move(uri_1.URI.revive(source), uri_1.URI.revive(target), opts.overwrite).catch(MainThreadFileSystem_1._handleError);
        }
        $copy(source, target, opts) {
            return this._fileService.copy(uri_1.URI.revive(source), uri_1.URI.revive(target), opts.overwrite).catch(MainThreadFileSystem_1._handleError);
        }
        $mkdir(uri) {
            return this._fileService.createFolder(uri_1.URI.revive(uri)).catch(MainThreadFileSystem_1._handleError);
        }
        $delete(uri, opts) {
