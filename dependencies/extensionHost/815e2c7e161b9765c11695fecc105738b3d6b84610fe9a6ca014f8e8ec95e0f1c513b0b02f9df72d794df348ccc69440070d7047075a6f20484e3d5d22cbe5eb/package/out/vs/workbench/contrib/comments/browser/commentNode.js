    ...
    let CommentNode = class CommentNode extends lifecycle_1.Disposable {
        ...
        createReactionsContainer(commentDetailsContainer) {
            ...
            this.comment.commentReactions.filter(reaction => !!reaction.count).map(reaction => {
                let action = new reactionsAction_1.ReactionAction(`reaction.${reaction.label}`, `${reaction.label}`, reaction.hasReacted && (reaction.canEdit || hasReactionHandler) ? 'active' : '', (reaction.canEdit || hasReactionHandler), () => __awaiter(this, void 0, void 0, function* () {
                    try {
                        yield this.commentService.toggleReaction(this.owner, this.resource, this.commentThread, this.comment, reaction);
                    }
                    catch (e) {
