        ...
        _updateItemClass(item) {
            ...
            if (item.iconLocation) {
                ...
                if (MenuEntryActionViewItem.ICON_PATH_TO_CSS_RULES.has(iconPathMapKey)) {
                    ...
                    dom_1.createCSSRule(`.icon.${iconClass}`, `background-image: ${dom_1.asCSSUrl(item.iconLocation.light || item.iconLocation.dark)}`);
                    dom_1.createCSSRule(`.vs-dark .icon.${iconClass}, .hc-black .icon.${iconClass}`, `background-image: ${dom_1.asCSSUrl(item.iconLocation.dark)}`);
