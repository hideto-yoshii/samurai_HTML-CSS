...
/*-----------------------------------------------------------------------------------------------
 *  this file is overridden because we need this patch: https://github.com/microsoft/vscode/commit/6b4d9860a519ca080264d19d5d282ec316f83e18
 *.  to upgrade to Node12
 *
 *  this is quickhack to get unblocked, proper solution is to upgrade vscode, see https://sim.amazon.com/issues/VFS-125
 *  once VS code is update this can be removed.
 *--------------------------------------------------------------------------------------------*/
				...
				.catch((error) => {
					try {
						if (nlsConfig._corruptedFile) {
							exports.writeFile(nlsConfig._corruptedFile, 'corrupted').catch(function (error) {
								console.error(error);
							});
						}
					} finally {
