...
// c9 changes: unnecessary dependency
...
const enabledExtensionsList = require('../extensions/enabled-extensions-list');
...
// c9 changes: filter by enabled extensions list
const tasks = compilations.filter((tsConfigFile) => enabledExtensionsList.enabledExtensions.indexOf(path.dirname(tsConfigFile)) >= 0).map(function (tsconfigFile) {
	...
	function createPipeline(build, emitError) {
		...
		const pipeline = function () {
			...
			const output = input
				...
				.pipe(compilation())
				// c9 changes: disabled localization
